package com.splus.testing.automation;
...
import com.splus.testing.automation.json.JSONHandler;
import com.sun.org.apache.xpath.internal.operations.Bool;

/*import Groupid.JsonProject.readJSONdata;*/
...
public class SmokeTesting {
	...
	public static String projectId,issueID,cycleID,jiraUrl,jiraPass,jiraUser;
	public static Boolean flag, testCasePassFailresult;
				...
				try{
					...
					if (!sFunction.isEmpty()) {
						if (row.getCell(4).getStringCellValue().matches(Constants.CONDITION_YES)) { // Should execute the test case or not
							...
							projectId=row.getCell(6).getStringCellValue();//Get ProjectId form Sheet
							issueID=row.getCell(7).getStringCellValue();//Get Execution Id from Sheet
							cycleID=row.getCell(8).getStringCellValue();//Get CycleID from Sheet
							jiraUrl=row.getCell(9).getStringCellValue();//Get JiraUrl from Sheet
							jiraUser=row.getCell(10).getStringCellValue();//Get JiraUser from Sheet
							jiraPass=row.getCell(11).getStringCellValue();//Get JiraPass from Sheet
							flag=row.getCell(12).getBooleanCellValue();//Get Flag value from Sheet
							...
							//driver.manage().timeouts().implicitlyWait(iTimeoutValue, TimeUnit.SECONDS);
							//Thread.sleep(3000);
							//Call this method if Test Cases are Pass
							if (Constants._PASSFAIL.equals("Pass"))
							{
								testCasePassFailresult = true;
							}
							else
							{
								testCasePassFailresult = false;
							}
							JSONHandler.createnewJSONfile(testCasePassFailresult,projectId,issueID,flag,cycleID,jiraUrl,jiraUser,jiraPass);
						} // Second If
					...
					//Call this method if Test Cases are Fail
					JSONHandler.createnewJSONfile(false,projectId,issueID,flag,cycleID,jiraUrl,jiraUser,jiraPass);
				}
